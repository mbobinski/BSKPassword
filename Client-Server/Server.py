...
#Algorytm El Gamal'a
def ELGServ(client):
    #Odbiór od klienta klucza publicznego
    test = client.recv(1024).split(',')
    #wylosowanie wartości do zaszyfrowania
    M = randint(1,int(test[2]))
    k = 127
    #szyforwanie wartości
    c1 = (int(test[1])**k) % int(test[2])
    c2 = (M*int(test[0])**k) % int(test[2])
    #przesłanie klientowi liczb do roszyfrowania
    client.send(str(c1)+","+str(c2))
    #sprawdzenie czy klient dobrze odpowiedział
...
def change_password(client,log,passw,newpassw):
    #sprawdzenie wszystkich utworzonych loginow
    for nick in login:
        #login poprawny
        if(nick == log):
            if(login[log] == passw):
...
while 1:
    ...
    #sprawdzenie czy klient to właściwa osoby, czy włamywacz
    test = ELGServ(client)
    if(test == 1):
